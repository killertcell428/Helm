...
Helmは、組織の意思決定プロセスを観測し、意思決定の詰まり（判断の遅れ・責任の曖昧さ等）を検知して改善を提案するAIシステムです。適応領域の詳細は [適応領域と制約](./docs/design/適応領域と制約.md) を参照。
...
定量的な指標に基づいて意思決定の詰まり（判断の遅れ・責任の曖昧さ等）を検知します：
...
各ロールは専用のプロンプトを使用し、それぞれの立場から判断が止まっているリスクを評価します。
...
**説明:**
「現在の会議は「正当化フェーズ」に入っています。数値悪化は共有されていますが、戦略変更を提案する主体と「やめる」という選択肢が事実上排除されています。」
