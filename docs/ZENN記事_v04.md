...
**デモURL（テキストリンク）**: [https://v0-helm-pdca-demo.vercel.app/](https://v0-helm-pdca-demo.vercel.app/)

#### 実装状況（Now / Next / Later）

| 区分 | 意味 | 主な項目 |
|------|------|----------|
| **Now** | デモで動く | データ取り込み（Meet/Chat）、検知・評価（ルール＋4ロールLLM）、アラート・承認、RACI/承認フロー、Document（Google Docs 認証時）、誤検知FB・精度指標API、監査ログ・保持期間 |
| **Next** | 2週間以内想定 | Google Chat 通知の実API、評価用ラベル蓄積・混同行列 |
| **Later** | 設計のみ | Firestore 永続化、Vertex AI Search、OIDC/IAP、管理UI、Calendar招集 |
...
| カテゴリ | 代表例 | できること | できないこと | Helmが唯一やること |
|----------|--------|------------|--------------|-------------------|
| 会議要約/議事録AI | Meet AI, Otter, Fireflies | 要約・議事録化・アクション抽出 | 責任・判断遅れの検知、誰にエスカレするか | 判断の質・責任・タイミングの観測と改善 |
| プロジェクト管理 | Jira, Asana, Monday | タスク・成果物の可視化 | 意思決定プロセス上の問題検知 | 同上 |
| 組織サーベイ | Culture Amp, 360度評価 | 組織サーベイ・満足度 | 会議・チャットからのリアルタイム検知 | 同上 |
| 意思決定ログ/ガバナンス | 議事録アーカイブ, Confluence | 記録・アーカイブ | 能動的なアラート・エスカレ | AIが人を呼び出す設計 |
| コンプラ監視 | 内部通報, 監査ツール | 違反検知・通報受理 | 意思決定プロセスの構造的問題 | 同上 |

Helmが唯一やるのは、**責任・判断・タイミング/質の観測と改善を能動的に行い、適切な人に判断を求めること**です。

**Helmが解決する3つの勝ちパターン（一点突破）**

| パターン | 説明 | 未解決である理由 |
|----------|------|------------------|
| ① 撤退/投資凍結判断の遅れ | KPI悪化が続くのに戦略変更・撤退議論がない（B1_正当化フェーズ） | 既存ツールは「記録」はするが、遅れを検知して誰に判断を求めるかはやらない |
| ② リスク報告の握り潰し | リスク認識があるが経営層への報告が遅延（ES1_報告遅延） | 会議要約は「何が言われたか」で、「言われていないこと」は検知できない |
| ③ RACI不整合の放置 | エスカレーション基準・責任の所在が曖昧（ES2_エスカレーション不明確） | 組織サーベイは事後調査であり、リアルタイムの不整合検知は対象外 |
...
Meet/Chat 取得から保存・閲覧まで、データの流れと役割別アクセスを図・表で示します。
...
flowchart LR
    ...
    subgraph ACCESS["閲覧（権限制御）"]
        GOV[ガバナンス担当]
        EXEC[経営層等]
    end

    MEET & CHAT --> ANON --> RULE & LLM
    RULE & LLM --> RAW & SIG & AUDIT
    SIG --> GOV & EXEC
    AUDIT --> GOV
    RAW -.->|ケース化＋二段階承認＋監査のみ| GOV
```

*図：データフロー（取得→マスキング→分析→保存→閲覧）。原文は最小保持・限定アクセス*

| 役割 | 原文（議事録・チャット） | シグナル（検知結果・スコア） | 監査ログ | 管理API（retention等） |
|------|--------------------------|------------------------------|----------|------------------------|
| 経営層・承認者 | × | ○ アラート・要約のみ | × | × |
| ガバナンス担当 | △ ケース化＋二段階承認＋理由入力＋監査時のみ | ○ | ○ | △ 監査範囲 |
| 分析実行（サービス） | ○ 取得・処理時のみ、7日で破棄 | ○ 書き込み・参照 | ○ 追記のみ | × |
| 人事評価 | × | × | × | × |

※ 人事と運用は分離。原文へのアクセスは「調査ケース化」された場合のみ、承認付きで延長・閲覧可能。
...
- **効果の定量評価（2つのKPI＋検証）**: **KPI①** 議題化までの日数（検知→承認→次回会議議題化）、**KPI②** 誤検知率（FP）と見逃し（FN）の運用コスト仮定。検証は匿名化疑似ログNケースで混同行列を算出し、アブレーション（ルールのみ／LLMのみ／ハイブリッド）で比較。詳細は設計ドキュメント [BENCHMARK_EVALUATION_DESIGN.md](https://github.com/killertcell428/Helm/blob/main/docs/design/BENCHMARK_EVALUATION_DESIGN.md) を参照
...
A: Helmは「生産性監視」ではなく、プロジェクト内の意思決定リスクを早期に見つける**安全管理のためのツール**です。個人行動の評価ではなく、**「意思決定プロセスの欠陥シグナル」の検知**であり、出力単位は原則**「案件／チーム」**です。ポイントは3つあります。
...
A: **企業ごとに重要な判断を順にパターンを追加していく**設計です。初回から網羅的なパターン定義は行いません。運用を重ねながら、重要な意思決定リスク（例：投資判断の見直し、報告遅延、現場乖離）に応じて段階的にパターンを定義・追加します。複数のパターンに該当する事象が検出された場合は、**事前に定義されたパターンの優先順位**（raci・approval_flows の定義順）に基づき、優先度の高い1件のみを発行。参照用のMECE枠組み（意思決定／報告・エスカレーション／執行・現場）を用意しており、必要に応じてパターンを拡張できます。
...
**ガードレール設計（実装／未実装）**：審査用に方針を明示します。

| 項目 | 状態 | 内容 |
|------|------|------|
| レート制限 | 実装済み | 1分あたりNリクエスト（`RATE_LIMIT_REQUESTS_PER_MINUTE`）。対象は `/api/` 配下。 |
| トークンバジェット（日次上限） | 実装済み | `LLM_DAILY_TOKEN_LIMIT` 超過時はモックフォールバック。 |
| リクエストサイズ制限 | 方針のみ | 巨大ペイロードによるメモリ枯渇を防ぐ上限を設計方針として設定予定（実装はNext）。 |
| DoS・悪用 | 方針 | レート制限で一部緩和。API Key漏洩時はローテーション・最小権限の運用で対応。コスト暴騰は日次トークン上限でガード。 |
