# "決裁待ち"を、過去にする。言えない本音を経営層へ届け推進する「Helm」
...
「決める人が不明」「決める期限がない」「反対意見が出ない」――この3つが揃うと、会議は動いて見えても意思決定は止まります。
Helmは会議・チャットからこの状態を検知し、**“決めるべき人”にアラートで呼び出し**、合意形成を進めるAIエージェントです。

既存のエージェントが「タスクを代行する」のに対し、Helmは **AIが人を呼ぶ**ことで、意思決定そのものを前に進めます。
...
- 未解決： 意思決定の詰まりを早期に検知して是正する仕組みがない。
- Helm：会議・チャットから「意思決定リスク」を発見し、経営層に判断を依頼（エスカレーション）するガードレールとして機能する（AIが人を呼ぶ）。

### 解決策の有効性

- 会議・チャットから「詰まり」を検知したら、**なぜ問題か**（根拠）と**誰に何を決めてもらうか**をまとめて、その人に承認を求めるまで一気通貫。
  ⇒　**「次に誰が何を決めるか」を具体的に落とす**ことで、滞留を解消する。

![](https://storage.googleapis.com/zenn-user-upload/d4a71235e53f-20260211.png)

**体験価値**：撤退判断の早期化、リスク報告の漏れ防止、炎上未然防止など
...
AIが社会・会社・組織に浸透していく真っ只中で、近い未来には個別のタスクや業務はほとんどが効率化され、逆に意思決定の時間や経営判断の遅延が推進の最も大きなボトルネックとして挙げられて来ることを強く実感しています。
- **事例**：パーソル総合研究所「無駄な社内会議による企業損失額調査」（2018年9月）によると、部長級管理職の週平均会議時間は**8.6時間**で、経営層のレビュー負荷は**週15時間**にも及びます[^5]。
  :::details 参考：Helmが解決する課題の調査事例
  従業員1,000名以上の企業では、以下の実データが報告されています。
  **意思決定リードタイムの遅延**
  株式会社IDEATECH「大企業・エンプラ（大企業向けシステム）の意思決定プロセス実態調査」（2025年6月）によると、新規サービス導入の意思決定期間は、平均で**約4〜5ヶ月（120〜150日）**にのぼります[^1]。戦略的意思決定と新規事業の意思決定を平均すると、**平均意思決定リードタイムは70〜85日**となります。
...
### 誰が何をいつ判断するかの組織課題
会社の中で「誰が何を判断するか」が問われる場面において、意思決定が遅延してしまう理由には大きく3つの課題が存在します。
...
既存のAIツールやプロジェクト管理ツール（Asana、Jira、CrewAI、AutoGenなど）は、タスクや成果物を最適化することに焦点を当てています。
...
これらは、タスク管理や成果物の最適化では解決できず、責任の所在や判断のタイミング・質を観測し、評価し、改善していくことが必要な問題です。

| 比較ツール | 既存ツール機能 | Helm機能 | Helmの強み |
|------------|----------------|----------|------------|
| **会議要約・議事録AI**（Otter、Fireflies、Gong等） | 文字起こし、要約、アクション抽出 | 議事録入力→判断リスク検知→人を呼び出す | 要約で終わらず、**検知→介入**まで一気通貫 |
| **プロジェクト管理**（Asana、Jira、Monday等） | タスク・成果物管理、進捗可視化、期限管理 | 会議・チャットから意思決定滞留を検知→人を呼ぶ | タスクではなく**意思決定プロセス**を観測 |
| **組織サーベイ**（Culture Amp、Lattice等） | 満足度・エンゲージメントの定期測定 | 議事録・チャット本文から判断リスク検知 | **満足度調査では得られない**やりとりデータを入力に |
| **意思決定ログ・ガバナンス**（意思決定記録ツール等） | 決定事項の記録、承認履歴管理 | 「決まっていない」「決める人がいない」を検知→人を呼ぶ | **決まった後**の記録ではなく、**決まる前**に介入 |
| **コンプラ・監査**（内部監査、コンプライアンス監視） | 規程違反・不正検知、監査証跡 | 意思決定遅れ・責任曖昧さを検知 | 「違反」ではなく**判断が止まっている**状態を検知 |

**Helmが唯一やること**：会議・チャットから「意思決定の詰まり」を検知し、**AIが人を呼び出して**「次に誰が何を決めるか」まで落とし込む一気通貫。既存ツールはカバーしていません。
...
Helmは、**あらかじめ決めた数値条件（ルールベース）と、AIによる複数視点評価（LLM）を組み合わせる**ことで、見落としのない統合判断を実現します。

ルールベース分析では、KPI下方修正回数、撤退議論の回数などの定量的指標に基づいて、安全側のベースライン評価を行います。一方、AIによる複数視点分析では、同じ会議ログとチャットログを、4つの異なる視点から評価します：

- 経営者視点：全社の業績・リスク・ステークホルダー責任の観点
- 経営企画視点：KPI・事業ポートフォリオ・撤退/投資判断の観点
- 現場視点：実行可能性と現場負荷の観点
- ガバナンス視点（法令遵守・内部統制）：報告遅延・隠れたリスク・コンプライアンスの観点

これらの評価結果を統合スコアにまとめることで、単一の評価軸では見落としがちな問題も、複数の視点から検知できるようになります。重みづけや評価設計の詳細は、後半の「POINT：ルール×LLMによるハイブリッド評価」で説明します。

### ② AIが人を呼び出し、判断を依頼

判断の遅れや責任の曖昧さなどを検知すると、Helmは自動で**役員 / 部長 / スタッフ**を特定し、判断・承認を依頼します。

人がAIを監視するのではなく、**AIが会議やチャットの状況を監視し、必要なときに人に判断を求める**という逆転の発想を実現しています。あらかじめ定義した「誰がどの判断に責任を持つか」（責任モデル）に基づいて適切な役割を決定し、「なぜこの人に判断を求めるべきか」を説明できる形で生成します。

### ③ 学習・改善PDCAの自動化

Helmは、以下のループ（PDCA：計画→実行→検証→改善のサイクル）を自動で回します：
...
これらを前提に、“組織にとって正当な介入”になるよう設計しています。

対象・対象外・制約の詳細は、GitHub の [適応領域と制約](https://github.com/killertcell428/Helm/blob/main/docs/design/適応領域と制約.md) を参照してください。
...
「①データ取り込み → ②検知・評価 → ③アラート発行・承認 → ④AI実行 → 保存」の順で進みます。
...
Google Meet APIから議事録を取得し、発言者抽出、KPI検出、撤退議論検出、リスク検出などの解析・構造化処理を実行し、JSONの構造化データに変換します。

:::details 補足：データ取得の条件（Google Meet / Chat）
- Meetの文字起こし（トランスクリプト）は、会議側で機能を有効にしている前提でAPIから取得
- Chatメッセージ取得は、対象スペース／スレッドに対する適切なアクセス権限を持つサービスアカウント前提
- 本記事では取得方法の詳細には踏み込みませんが、「どの会議・どのスペースを対象にするか」という範囲・権限の設計が実運用では重要
:::
...
あらかじめ決めた数値条件（ルールベース）と、AIによる複数視点分析（LLM）で問題特定を行います。
ハイブリッド評価の設計思想や重みづけの詳細は、後半の「POINT：ルール×LLMによるハイブリッド評価」を参照してください。

#### ③アラート発行

あらかじめ定義した「誰がどの判断に責任を持つか」（責任モデル）に基づき、AIが人を呼び出します。

判断の遅れや責任の曖昧さなどが検知されると、Helmは自動的に適切な役割（経営層など）を決定し、判断依頼の理由を生成します。

:::details 組織グラフや承認フローはどう用意する？
導入時に、次の3つを会社の実態に合わせてヒアリングし定義します。

1. **組織グラフ**：誰が誰に報告するか（報告関係）
2. **RACI**：どの判断に誰が責任を持つか（責任分担）
3. **承認フロー**：判断依頼を誰が誰の承認を経て進めるか（例：投資凍結＝CFO承認→経営層承認）

これらは**JSONの定義ファイルで設定**し、Helmが「この問題ならこの役割の人に判断を求める」「この種の介入ならこの承認ルートを使う」と自動で決めます。多段階承認にも対応しています。

定義が未設定の場合は、経営層への1回承認で代用します。現在はファイルで管理しており、将来的にはデータベース保存や管理画面での編集を想定しています。
:::
...
経営層の承認後、HelmはADK（Agent Development Kit）を用いた複数エージェント連携で自律的にタスクを実行します。具体的には、市場データ分析、社内データ統合、3案比較資料の自動生成、関係部署への事前通知、会議アジェンダの更新などを行います。

:::details 補足：AI自動実行はこの機能しか使えない？
Helmは、あくまで意思決定から自動でアクションまで紐づいた連続性を機能の中心においているため、タスク実行本体の機能については簡易な実装になります。各タスクの実行については、会社ごとに必要な要件やコストを踏まえた連携・実装を想定しています。
:::
...
議事録の解析では発言者抽出・KPI検出・撤退議論検出、チャットの解析ではリスク検出・判断依頼の兆候検出・反対意見検出を行い、構造化データとして保持します。

#### 2. 判断・責任にまつわる問題の検知フロー（複数視点評価システム）

| ツール・サービス                 | 役割                                                                           |
| -------------------------------- | ------------------------------------------------------------------------------ |
| **StructureAnalyzer**      | ルールベース分析（KPI下方修正回数、撤退議論の有無、判断集中率、反対意見無視）  |
| **MultiRoleLLMAnalyzer**   | 4役割（経営者／経営企画／現場／ガバナンス）から同一データをAI（LLM）で評価 |
| **EnsembleScoringService** | 0.6×ルール + 0.4×LLM で統合、重要度・緊急度は安全側を採用                    |
| **ScoringService**         | 各検知結果の重要度・緊急度評価                                                |
...
| **データ・定義**     | JSON定義 / Firestore（設計） | 役割と承認のルール（組織グラフ・RACI・承認フロー）は `config/definitions/` の JSON で管理。永続化・共有は Firestore を想定した設計 | 組織グラフ（報告関係）・RACI・承認フローテンプレートはファイルで管理。検知記録・エスカレーション履歴の永続化は設計段階                                                                                              |
...
#### 補足：GCP構成
**Cloud Run**（API・ADKエージェント）、**Firestore**（役割・承認ルールの保存・検知記録、設計段階）、**Pub/Sub**（イベント駆動・ジョブキュー）、**Vertex AI**（Gemini）、**BigQuery**（メトリクス・分析）、**Cloud Logging / Monitoring**（監査・運用）。Secrets Manager で認証情報を管理する設計です。

### 運用・非機能要件（スケール/将来実装）

本記事では主にアーキテクチャとLLMまわりにフォーカスしていますが、実際にHelmを運用していく上では次のような非機能要件も重要になります。

- **認証・認可**: 誰がHelmを実行できるか（役割に応じたアクセス制御）、どの会議／チャットにアクセスできるか
- **テナント分離**: 事業部・グループ会社ごとにデータを分離し、組織ごとのポリシーを設定する仕組み
...
- **コスト上限**: LLM呼び出しごとにトークン使用量を記録し、日次累積で上限チェック。環境変数 `LLM_DAILY_TOKEN_LIMIT` を超えた場合はモック（模擬）応答に切り替え。
- **監査ログの改ざん耐性**: 各ログエントリに `prev_hash`・`entry_hash` のハッシュチェーンを付与。`GET /api/audit/verify` でチェーン検証が可能。
- **分析メトリクス**: 会議1件あたりの処理時間（レイテンシ）・AI呼び出し回数・トークン数（AIの処理単位）を記録。`GET /api/metrics/usage` で直近の平均処理時間・累積トークン数・分析件数を取得可能。
...
| 領域 | 内容 |
|------|------|
| **データ取り込み** | Google Meet / Chat / 会議資料の ingest API。解析・構造化まで一通り対応。 |
| **検知・評価** | ルールベース（StructureAnalyzer）＋AIによる複数視点分析（4役割）＋統合スコア。パターン検出でアラート生成。 |
| **アラート・承認** | EscalationEngine（閾値・重要度）、`POST /api/escalate`・`POST /api/approve`。組織グラフ・RACI（責任分担）・承認フローは JSON 定義で制御（後述）。 |
| **AI自律実行** | ADK ベースの複数エージェント連携（調査／分析／通知／資料作成）。WebSocket で進捗配信。2月時点ではモック＋代替応答まで。 |

#### ガバナンス・運用まわり

| 項目 | 仕組み |
|------|--------|
| **認証** | API Key ＋ 役割（`X-API-Key`）。環境変数 `API_KEYS` で有効化。プロダクションでは OIDC（認証標準）／IAP（アクセス制御）／Workspace 連携の方向性を想定。 |
...
| **取得範囲・アラート抑制** | ホワイトリスト（会議/チャットID）と抑制条件（パターン＋リソース）で制御。 |
...
| **冪等性** | 同じ承認に対して実行が二重に走らないよう防ぐ。設計は [idempotency-execute.md](https://github.com/killertcell428/Helm/blob/main/docs/idempotency-execute.md)。 |
| **レート制限・コスト上限** | レート制限は1分あたりNリクエスト（`RATE_LIMIT_REQUESTS_PER_MINUTE`）。コスト上限は日次トークン数（`LLM_DAILY_TOKEN_LIMIT`）で制御。環境変数で設定可能。 |
| **分析メトリクス** | `GET /api/metrics/usage` で直近の分析の平均処理時間・トークン数・分析件数を取得。 |
...
- **永続化**：検知記録・判断依頼履歴の DB/Firestore 保存
...
| 通知ポリシー | [notification-policy.md](https://github.com/killertcell428/Helm/blob/main/docs/future/notification-policy.md) | 役割別通知・チャネル（メール／Slack 等）。 |


### 実装POINT：ルール×LLMによるハイブリッド評価

Helmは、ルールベース分析とAI（LLM：大規模言語モデル）による複数視点分析を組み合わせることで、**判断の遅れや責任の曖昧さ**といった問題を、より信頼性高く検知できるようにしています。なぜこの2つを組み合わせる必要があるのか、それぞれの限界と利点を説明します。
...
AIによる複数視点分析は、4つの役割視点から同一データを評価することで、文脈理解により過剰反応を抑制します。
...
しかし、AI（LLM）単体にも限界があります。楽観バイアス・ハルシネーション（誤った内容を生成してしまうこと）のリスクがあり、過剰なアラートを発する可能性があります。
...
- **説明文**: ルールベースの説明と主要な役割のコメントを統合

重要度・緊急度は、ルールベースと各役割の結果のうち、**最も安全側（最も強い）の評価を強制採用**します。これにより、過小評価を避け、かつ過剰なアラートも避けるバランスを取っています。
...
- **精度指標のモニタリング**: 適合率（検出した問題のうち実際に問題だった割合）／再現率（実際の問題のうち検出できた割合）を追い、過剰アラートになっていないか定期的に検証する（`GET /api/metrics/accuracy` で取得、誤検知は `POST /api/feedback/false-positive` で登録）
- **定量インパクトの測定設計**: 意思決定リードタイム短縮の仮説値として、IDEATECH調査（平均70〜85日）をベースに、Helm介入により「経営層への早期判断依頼→議題化」で**2〜4週間の短縮**を1ケースで検証する設計。校正データ（匿名化疑似ログ）で偽陽性・真陽性を事前評価
- **アラート抑制**: 同種のアラートが一定時間内に繰り返し出た場合は、1件にまとめて通知する（パターンID＋リソース単位の抑制設定で制御）
- **役割別の通知ポリシー**: 経営層には「本当に重要度の高いもの」だけを通知し、詳細は経営企画・ガバナンスチームでレビューする
...
1. **AIが人を呼び出す**: 判断の遅れや責任の曖昧さなどを検知すると、自動的に適切な役割（経営層など）を呼び出す
2. **多角的評価**: 数値条件（ルールベース）と AI（LLM）を組み合わせた評価で、4つの視点（経営者、経営企画、現場、ガバナンス）から同時に分析
...
**対象外**として、1on1・私的DM・メンタル相談・労組窓口・内部通報窓口などは「絶対に見ない」と先に宣言。人事担当と運用担当は別の役割に分け、混在しない設計です。現状は誤検知フィードバック・監査ログ・マスキング・アクセス制御を実装済み。**原文の超短期破棄**は環境変数（`RETENTION_DAYS_MEETINGS` 等）で7日破棄を設定可能（実装済み）。社内規程への落とし込みなどは検討中です。
...
A: 一般的なチャットボットは「人がAIを呼び出す」形です。Helmは逆に「**AIが人を呼び出す**」形です。会議やチャットを横断して、意思決定の遅れや責任の曖昧さといった「意思決定の詰まり」を検知し、適切な担当者（経営層など）に判断を依頼します。タスクの自動化ではなく、**意思決定のプロセスそのもの**に働きかける点が特徴です。
...
A: REST APIとWebSocketで完結しており、フロントエンド（Next.js）はAPIクライアントとして分離されています。同一APIでPWAやモバイルアプリなどへの展開も可能な設計です。認証は現状API Key＋役割による制御。OIDC（認証標準）／IAP（アクセス制御）やWorkspace連携は将来の拡張として設計上の余地を確保していますが、未実装です。
...
A: 現状はAPI Keyと役割による認証を実装済みです。プロダクション向けのOIDC（認証標準）／IAP（アクセス制御）・Workspace連携は未実装。会議・チャットへのアクセス権限は役割で分離しています。**脅威モデル・セキュリティ設計**はREADMEに簡易脅威モデルとセキュリティ設計TODO（優先度付き）を記載済みです。

**Q: レート制限やコスト暴騰、プロンプトインジェクションへの対策は？**

A: **レート制限・コスト上限は実装済み**です。レート制限は1分あたりNリクエスト（環境変数 `RATE_LIMIT_REQUESTS_PER_MINUTE`）、コスト上限は日次トークン数（環境変数 `LLM_DAILY_TOKEN_LIMIT`）で制御。超えた場合は429エラーまたはモック（模擬）応答で対応します。AIへの入力は構造化データに限定し、ユーザーの自由入力は経由しない設計のため、プロンプトインジェクション（悪意ある入力でAIの挙動を不正に変える攻撃）は設計上抑止されています。**監査ログの改ざん耐性**は各エントリにハッシュチェーンを付与し、`GET /api/audit/verify` で検証可能です。
...
[^6]: https://www.ppc.go.jp/all_faq_index/faq1-q5-7
