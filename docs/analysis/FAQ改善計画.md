...
**目的**: ZENN記事のFAQセクションにおいて、曖昧な回答を排除し、未設計・検討中は明示、設計を詰めるべき項目は具体的な検討内容を整理する。適応領域・対象外・制約の正式な定義は [適応領域と制約](../design/適応領域と制約.md) を参照。
...
| 役割・承認ルール（RACI等）のメンテ | 管理UIは未実装、JSON編集のみ | 「**現状は** `config/definitions/` のJSONを直接編集。Firestore永続化後は管理UIでの編集を想定しているが、**管理UIは未実装。** バージョン管理・監査ログによる変更履歴追跡は設計済み。」 |
